'''
- die wichtigsten Änderungen von Version 006 zu 007:

- neue Class ScrollFog: Scrollt ein Bild abwechselnd mit eigenem Spiegelbild (flip_horizontally)
- ScrollFog wiederholt endlos, Bild ist idealerweise min. 1000 hoch und möglichst 1000 breit

- spacefog wieder entfernt weil scrollfog das mit übernimmt,
    beliebig viele scrollfog-hintergründe jetzt über listen in der ScrollFog.setup ansteuerbar

- torpedo feuern ist jetzt eine methode der class Torpedo
- torpedo feuern nun auch mit SPACE möglich
'''
...
class ScrollFog():
    ...
    def setup(self):
        ...
        self.scrollfog_data = [["container/1000_0b_gimp.png", .4, 100],
                               ["container/1000_2_gimp.png", .1, 50],
                               ["container/1000_0_gimp.png", .01, 100]
                               ]
        anzahl = (len(self.scrollfog_data))
        # leere 2-dim Liste erstellen, geht das auch anders???
...
# TODO ... zB. eine Füllfunktion wie oben allen Feldern einmal "egal was" oder auch garnix  zuweist!?
        for i in range(anzahl):
            self.fog_file = self.scrollfog_data[i][0]
            self.scrollfog_speed = self.scrollfog_data[i][1]
            self.scrollfog_alpha = self.scrollfog_data[i][2]
